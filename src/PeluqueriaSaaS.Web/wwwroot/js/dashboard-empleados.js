// Chart para empleados
let empleadosChart;

document.addEventListener('DOMContentLoaded', async function() {
    console.log('📊 Inicializando Dashboard con métricas de empleados');
    
    const data = await loadDashboardData();
    if (data && !data.error) {
        initializeAllCharts(data);
        initializeEmpleadosChart(data.ventasPorEmpleado);
    }
});
...
function initializeEmpleadosChart(empleadosData) {
    const ctx = document.getElementById('empleadosChart');
    if (!ctx || !empleadosData) return;
    
    empleadosChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: empleadosData.map(e => e.empleadoNombre),
            datasets: [{
                label: 'Ventas del Mes ($)',
                data: empleadosData.map(e => e.totalVentas),
...
function initializeAllCharts(data) {
    // Copiar las funciones del dashboard.js original aquí
...
// Resto de funciones del dashboard.js original...
    /*
